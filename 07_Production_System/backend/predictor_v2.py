...
# Risk-band methodology (Sprint 9B): fixed business thresholds, chosen as
# the single documented V2 standard after a prior audit found this file's
# percentile-derived cutoffs (originally 0.0354 / 0.8687, from the 25th/90th
# percentile of predicted probabilities on the training set) disagreed with
# 04_Gradio/gradio_app_v2.py's simple fixed thresholds -- the same model
# probability could be labeled "High Risk" in one interface and "Medium
# Risk" in the other. Fixed thresholds were chosen over re-deriving new
# percentile cutoffs because they are simple to state to a non-technical
# underwriting audience, do not silently shift every time the model is
# retrained (percentile cutoffs would move whenever the score distribution
# changes, even if nothing about the risk policy changed), and are already
# the standard gradio_app_v2.py uses -- so unifying meant changing this file,
# not that one. Any future change to these two numbers must be made in both
# places; see docs/production/risk_scoring_framework_v2.md for the full
# methodology writeup and the cross-interface consistency check.
