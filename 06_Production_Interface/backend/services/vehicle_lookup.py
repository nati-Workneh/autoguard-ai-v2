...
class VehicleLookupService:
    ...
    def _fetch_with_retry(self, request: urllib_request.Request) -> dict[str, Any]:
        """Perform the actual HTTP call with a small bounded retry.

        Only retries transport-level failures (timeout, HTTP error, an
        unparseable response) -- never retries a legitimate business result
        (e.g. the plate simply not existing), and never fabricates a
        result: the last attempt's real error is what gets raised.
        """
        last_error: VehicleLookupError | None = None
        for attempt in range(1, REGISTRY_MAX_ATTEMPTS + 1):
            try:
                with urllib_request.urlopen(request, timeout=self.timeout_seconds) as response:
                    return json.load(response)
            except urllib_error.HTTPError as exc:
                last_error = VehicleLookupUpstreamError(
                    f"vehicle registry request failed with HTTP {exc.code}"
                )
            except urllib_error.URLError as exc:
                if isinstance(exc.reason, (TimeoutError, socket.timeout)):
                    last_error = VehicleLookupTimeoutError("vehicle registry request timed out")
                else:
                    last_error = VehicleLookupUpstreamError("vehicle registry request failed")
            except TimeoutError:
                last_error = VehicleLookupTimeoutError("vehicle registry request timed out")
            except socket.timeout:
                last_error = VehicleLookupTimeoutError("vehicle registry request timed out")
            except json.JSONDecodeError:
                last_error = VehicleLookupUpstreamError("vehicle registry returned invalid JSON")
