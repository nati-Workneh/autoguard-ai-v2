...
def test_low_risk_yields_discount_within_business_range() -> None:
    estimate = estimate_premium_impact(
        risk_level="Low", claim_probability=0.05, low_cutoff=LOW_CUTOFF, high_cutoff=HIGH_CUTOFF,
    )
...
def test_medium_risk_yields_standard_premium_with_no_change() -> None:
    estimate = estimate_premium_impact(
        risk_level="Medium", claim_probability=0.45, low_cutoff=LOW_CUTOFF, high_cutoff=HIGH_CUTOFF,
    )
...
def test_high_risk_yields_surcharge_within_business_range() -> None:
    estimate = estimate_premium_impact(
        risk_level="High", claim_probability=0.9, low_cutoff=LOW_CUTOFF, high_cutoff=HIGH_CUTOFF,
    )
...
def test_low_risk_discount_is_largest_at_lowest_probability(
    ...
    estimate = estimate_premium_impact(
        risk_level=risk_level, claim_probability=claim_probability, low_cutoff=LOW_CUTOFF, high_cutoff=HIGH_CUTOFF,
    )
...
def test_high_risk_surcharge_is_largest_at_highest_probability() -> None:
    near_cutoff = estimate_premium_impact(
        risk_level="High", claim_probability=HIGH_CUTOFF + 0.001, low_cutoff=LOW_CUTOFF, high_cutoff=HIGH_CUTOFF,
    )
    near_one = estimate_premium_impact(
        risk_level="High", claim_probability=0.99, low_cutoff=LOW_CUTOFF, high_cutoff=HIGH_CUTOFF,
    )
...
def test_summary_text_is_non_empty() -> None:
    for risk_level in ("Low", "Medium", "High"):
        estimate = estimate_premium_impact(
            risk_level=risk_level, claim_probability=0.5, low_cutoff=LOW_CUTOFF, high_cutoff=HIGH_CUTOFF,
        )
