...
ROOT=Path(__file__).resolve().parents[1];HERE=Path(__file__).resolve().parent;PROC=ROOT/'02_Data/processed';FIG=ROOT/'05_Report/figures/sprint3_1'
def json_safe(value):
...
def main():
 FIG.mkdir(parents=True,exist_ok=True);a=assumptions();m=json.loads((ROOT/'03_Model/model_v2_metadata.json').read_text());pipe=joblib.load(ROOT/'03_Model/model_v2.pkl');va=pd.read_csv(PROC/'real_validation_sprint1.csv');te=pd.read_csv(PROC/'real_test_sprint1.csv');p=pipe.predict_proba(va[m['feature_list']])[:,1];exp=scenario(a,'Expected');rows=[]
 ...
 fig,ax=plt.subplots(figsize=(8,5));ax.plot(grid.threshold,grid.annual_net_benefit);ax.axvline(m['threshold_analysis']['selected_statistical_threshold'],ls='--',label='Statistical threshold');ax.axvline(bt,ls=':',label='Business threshold');ax.legend();ax.set(title='Validation business threshold economics',xlabel='Threshold',ylabel='Expected annual net benefit (USD)');fig.tight_layout();fig.savefig(FIG/'threshold_economic_value.png',dpi=180);plt.close(fig)
 ...
 out=json_safe({'model_version':m['version'],'artifact_sha256':hashlib.sha256((ROOT/'03_Model/model_v2.pkl').read_bytes()).hexdigest(),'statistical_threshold':m['threshold_analysis']['selected_statistical_threshold'],'business_threshold':bt,'business_threshold_selection_dataset':'real_validation','threshold_selection_dataset':'real_validation','scenarios':sc.to_dict('records'),'test_observed_confusion_matrix':dict(zip(['tn','fp','fn','tp'],map(int,tcm))),'test_routing':test,'break_even':{'minimum_annual_volume_year1_roi_nonnegative':be,'maximum_implementation_cost_year1_roi_nonnegative':calculate_implementation_cost_break_even(e),'critical_fn_cost_annual_net_benefit_zero':critical_net,'critical_fn_cost_year1_roi_zero':critical_year1,'year1_fn_cost_break_even_interpretation':describe_year1_fn_cost_break_even(critical_year1)}})
 policy={'policy_version':'v1.0-sprint3.3','business_threshold':bt,'statistical_threshold':m['threshold_analysis']['selected_statistical_threshold'],'manual_review_required_at_or_above_business_threshold':True,'human_review_required_for_high_risk':True,'selection_dataset':'real_validation','selection_objective':'maximize_expected_annual_net_benefit','economic_model_version':'sprint3.3','source':'Sprint 3 economic optimization'}
