"""Sprint 7.6/7.7 Gradio inference interface for AutoGuard AI.
...
# Approved portfolio defaults for the 4 fields hidden in Agent Mode.
# Source: data/raw/train.csv (58,592 labeled rows, the approved dataset).
# Computed the same way as HIDDEN_FIELD_DEFAULTS below (median for numeric,
# mode for categorical/binary). Documented in
# docs/reports/agent_mode_simplification.md.
...
.detail-value {margin: 0; font-size: 15px; color: #1b2f49; font-weight: 600;}
...
def _build_input_component(field: FormField, label_override: str | None = None, default_value: Any = None) -> gr.Component:
    ...
    return gr.Number(
        label=label,
        info=field.help_text,
        minimum=float(field.min) if field.min is not None else None,
        maximum=float(field.max) if field.max is not None else None,
        step=float(field.step or 1),
        precision=precision,
